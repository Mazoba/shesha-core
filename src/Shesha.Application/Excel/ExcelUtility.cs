...
namespace Shesha.Excel
{
    public class ExcelUtility: IExcelUtility, ITransientDependency
    {
        ...
        public ExcelUtility(ReferenceListHelper refListHelper)
        {
        ...
        private Func<object, string> CreateGetAsString(Type rowType, ExcelColumn c)
        {
            ...
            if (ReflectionHelper.IsReferenceListProperty(property))
            {
                ...
                var list = _refListHelper.GetItems(refListIdentifier.Namespace, refListIdentifier.Name);
                ...
                if (isMultiValue)
                {
                    return new Func<object, string>(val =>
                    {
                        ...
                        var items = _refListHelper.DecomposeMultiValueIntoItems(refListIdentifier.Namespace, refListIdentifier.Name, Convert.ToInt64(val));
