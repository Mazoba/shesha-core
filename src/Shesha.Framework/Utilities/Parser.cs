...
namespace Shesha.Utilities
{
    ...
    public class Parser
    {
        public static bool TryParseToMultiValueReferenceListValue(string value, out int? parsedValue, string referenceListNamespace, string referenceListName)
        {
            Contract.Requires(!string.IsNullOrWhiteSpace(referenceListNamespace));
            Contract.Requires(!string.IsNullOrWhiteSpace(referenceListName));
        ...
        {
            return TimeSpan.TryParseExact(value, format ?? @"hh\:mm"/*note: `hh` is always 24-hours for TimeSpan*/, CultureInfo.InvariantCulture, TimeSpanStyles.None, out var timeSpan)
                ? (TimeSpan?)timeSpan
                : null;
        }
        ...
        public static DateTime ParseDate(string value, string format = null, bool isDateOnly = true)
        {
            ...
            else if (string.IsNullOrEmpty(format))
            {
                ...
                if (isDateOnly)
                {
                    DateTime.TryParseExact(value, UISettings.DefaultDateFormat(isForEdit: true),
                                           CultureInfo.InvariantCulture, DateTimeStyles.None, out dateVal);
                }
                else
                {
                    parseSuccessful = DateTime.TryParseExact(value, UISettings.DefaultDateTimeFormat(isForEdit: true), CultureInfo.InvariantCulture, DateTimeStyles.None, out dateVal);
                    // try to parse only date if time is not posted to the server
                    if (!parseSuccessful)
                        parseSuccessful = DateTime.TryParseExact(value, UISettings.DefaultDateFormat(isForEdit: true), CultureInfo.InvariantCulture, DateTimeStyles.None, out dateVal);
                }
            }
        ...
        internal static bool TryParseToMultiValueReferenceListPropertyType(string value, PropertyInfo propInfo, out object parsedValue)
        {
            ...
            int? parsedValueInt;
            var res = Parser.TryParseToMultiValueReferenceListValue(value, out parsedValueInt, propConfig.ReferenceListNamespace, propConfig.ReferenceListName);
