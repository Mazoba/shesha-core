...
namespace Shesha.Import
{
    ...
    public abstract class ImportServiceBase<T>: IImport<T> where T : ImportResult
    {
        ...
        public void Import(Guid importResultId, CancellationToken cancellationToken)
        {
            ...
            {
                /*
                if (string.IsNullOrWhiteSpace(importResult.LogFilePath))
                    importResult.LogFilePath = Path.Combine(this.Resolve<ISheshaSettings>().UploadFolder, $"{Path.GetTempPath()}{DateTime.Now:yyyyMMdd-hh.mm.ss}_import.log");
                */

                result.StartedOn = DateTime.Now;
            });

            ConfigureAppenders(tempLogFile);
            try
            {
                try
                {
        ...
        public async Task ImportAsync(string fileName, Stream stream, CancellationToken cancellationToken, Action<T> prepareResultAction)
        {
            ...
            {
                // Create log file
                //var logFileName = Path.Combine(SheshaSettings.UploadFolder, $"{Path.GetFileNameWithoutExtension(fileName)}.log");
                ...
                using (var uow = _unitOfWorkManager.Begin(scope: TransactionScopeOption.RequiresNew))
                {
                    ...
                    importResult.ImportedFile = await _fileService.SaveFile(stream, fileName);
                    importResult.ImportedFileMD5 = FileHelper.GetMD5(stream);
                    //importResult.LogFilePath = logFileName;
