...
namespace Shesha.JsonLogic
{
    ...
    /// JsonLogic operators
    /// </summary>
    public class HqlManageOperators : IManageOperators
    {
        private Dictionary<string, Func<IProcessJsonLogic, JToken[], object, object>> registry;

        public static HqlManageOperators Default { get; } = new HqlManageOperators();

        public HqlManageOperators()
        {
        ...
        private void AddDefaultOperations()
        {
            ...
            AddOperator("in", (p, args, data) => {
                ...
                if (haystack is string s)
                    return !string.IsNullOrEmpty(needle.ToString()) && s.IndexOf(needle.ToString() ?? string.Empty, StringComparison.Ordinal) >= 0;
